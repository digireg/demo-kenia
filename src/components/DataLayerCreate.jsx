...
export async function DataLayerCreate(wmsUrl, datasetName = "WMS Layer") {
  ...
  try {
    ...
    //   rootLayer.querySelectorAll(":scope > Layer")
    // )
    //   .map((layerNode) => traverseLayer(layerNode))
    //   .filter(Boolean);
    const firstLevelLayers = Array.from(
      rootLayer.querySelectorAll(":scope > Layer")
    )
...
//       return {
//         id: name || title,
//         key: `${safeDatasetName}:${name || title}`,
//         name: name || title,
//         title,
//         inputType: "checkbox",
//         type: "wms",
...
//         sourceType: safeDatasetName.toLowerCase() === "bag" ? "bag" : "wms",
//         legendUrl,
//         groupTitle: safeDatasetName, // top-level only
//       };
//     });

//     return topLayers;
//   } catch (err) {
//     console.error("Error fetching WMS GetCapabilities:", err);
//     return [];
//   }
// }
